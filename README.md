...
│   ├── providers.tf           # AWS and Kubernetes provider declarations
│   ├── variables.tf           # Configuration variables
│   ├── main.tf                # VPC, subnets, NAT Gateway, EKS Cluster, node groups
...
### Provisioning Network & Cluster (Terraform)
...
Apply resources inside EKS or local cluster (Minikube / Docker Desktop):
```bash
# Create namespace
...
kubectl create secret generic chronosai-db-secrets \
  --from-literal=username=chronos_readwrite_user \
  --from-literal=password=SuperSecureAutogeneratedPasswordHash98124 \
  -n chronosai

# Deploy configurations and pods
kubectl apply -f kubernetes/
```
Verify Horizontal Pod Autoscaling (HPA) tracking:
```bash
kubectl get hpa -n chronosai -w
```
